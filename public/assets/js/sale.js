...
$('.products-body').on('change', '.row_product', function () {
        ...
        $('.products-body').on('input', '.row_quantity, .row_cost, .row_discount, .row_discount_type', function () {
            let $currentRow = $(this).closest('tr');
            $('.data').addClass('load');
                ...
                if (discountType === 'fixed') {
                    total = (cost - discount) * quantity;
                } else if (discountType === 'percent') {
                    let discountAmount = ((cost * discount)) / 100;
                    total = (cost - discountAmount) * quantity;
                } else {
                    total = cost * quantity;
                }
                ...
                updateSummarySale();
                $('.data').removeClass('load');
                $currentRow.find('.row_quantity, .row_cost, .row_discount, .row_discount_type').prop('disabled', false);
        });
